...
package net.mcreator.extraequipment.world.biome;
...
import net.mcreator.extraequipment.entity.JtrkrtEntity;
...
public class AzshuraBiome extends ExtraEquipmentModElements.ModElement {
	...
	public void init(FMLCommonSetupEvent event) {
		...
		BiomeManager.addBiome(BiomeManager.BiomeType.WARM, new BiomeManager.BiomeEntry(biome, 124));
	}
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0.5f).depth(0.1f).scale(0.2f).temperature(0.5f).precipitation(Biome.RainType.RAIN)
					.category(Biome.Category.SWAMP).waterColor(-15122169).waterFogColor(-15122169).parent("swamp")
					.surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(DeadgrassBlock.block.getDefaultState(),
							Blocks.STONE.getDefaultState(), Blocks.STONE.getDefaultState())));
			setRegistryName("azshura");
			...
			this.addSpawn(EntityClassification.MONSTER, new Biome.SpawnListEntry(EntityType.HUSK, 15, 4, 4));
			this.addSpawn(EntityClassification.AMBIENT, new Biome.SpawnListEntry(EntityType.BAT, 5, 2, 2));
			this.addSpawn(EntityClassification.MONSTER, new Biome.SpawnListEntry(EntityType.WITCH, 13, 4, 4));
			this.addSpawn(EntityClassification.MONSTER, new Biome.SpawnListEntry(JtrkrtEntity.entity, 3, 1, 1));
		}
	}
		...
		protected boolean place(IWorldGenerationReader worldgen, Random rand, BlockPos position, Set<BlockPos> changedBlocks,
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == DeadgrassBlock.block.getDefaultState().getBlock() || ground == Blocks.STONE.getDefaultState().getBlock())
							&& (ground2 == DeadgrassBlock.block.getDefaultState().getBlock()
								...
								for (Direction Direction : Direction.Plane.HORIZONTAL) {
									if (rand.nextInt(4 - hlevel) == 0) {
										...
										setTreeBlockState(changedBlocks, world, position.add(dir.getXOffset(), height - 5 + hlevel, dir.getZOffset()),
												Blocks.SWEET_BERRY_BUSH.getDefaultState(), bbox);
									}
								}
							}
						}
